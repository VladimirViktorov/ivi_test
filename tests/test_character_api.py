import pytest
import allure
from assertions.base.solutions import assert_status_code, assert_error, assert_len_database, assert_error_data, assert_result
...
from constant import APIErrors, APIResults
...
class TestCharacter:
    ...
    @pytest.mark.regression
    @allure.title("TC-020")
    @allure.description("Deleting information about a character")
    @pytest.mark.parametrize('name', ['Agent Brand',
                                      'Ancient One (Ultimate)',
                                      'Anole'])
    def test_delete_info_about_character(self, class_character_client: CharacterClient, reset_database, name: str):
        response = class_character_client.delete_character_api(name)
        check_response = class_character_client.get_character_api(name)
        
        assert_status_code(response.status_code, 200)
        assert_result(response.json().get('result', {}), APIResults.DELETE_CHARACTER.substitute(name=name))
        assert_status_code(check_response.status_code, 400)
        assert_error(check_response.json().get('error', {}), APIErrors.NO_SUCH_NAME)
    
    @pytest.mark.regression
    @allure.title("TC-021")
    @pytest.mark.need_review
    @allure.description("Deleting information about a non-existent character")
    @pytest.mark.parametrize('name', ['Test'])
    def test_delete_info_about_non_existent_character(self, class_character_client: CharacterClient, reset_database, name: str):
        response = class_character_client.delete_character_api(name)
